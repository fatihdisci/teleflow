...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    {setupOpen && <div className="modal-backdrop" onClick={() => setSetupOpen(false)}><div className="setup-modal" onClick={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => setSetupOpen(false)}>×</button><div className="modal-icon">T</div><div className="eyebrow">GÜVENLİ KURULUM · ADIM {step}/3</div><h2>{step === 1 ? "Telegram API bilgilerinizi ekleyin" : step === 2 ? "Telefon numaranızı doğrulayın" : "Bağlantı hazır"}</h2><p>{step === 1 ? "my.telegram.org → API development tools sayfasından api_id ve api_hash alın. Bu bilgiler bot tokenı değildir; kullanıcı hesabınızın güvenli erişim anahtarıdır." : step === 2 ? "Telefon numaranızı ülke koduyla yazın. Doğrulama kodu Telegram uygulamanıza gönderilir; kod tarayıcıya kaydedilmez." : "Bağlantı bilgileri şifreli olarak sunucuda tutulur. Artık komut akışlarını çalıştırabilirsiniz."}</p>{step === 1 && <div className="form-grid"><label>api_id<input placeholder="Örn. 12345678" /></label><label>api_hash<input placeholder="32 karakterlik değer" type="password" /></label></div>}{step === 2 && <label>Telefon numarası<input value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="+90 5xx xxx xx xx" /></label>}<div className="modal-actions"><button className="secondary-button" onClick={() => setSetupOpen(false)}>İptal</button><button className="primary-button" onClick={() => setStep(step === 3 ? 1 : step + 1)}>{step === 3 ? "Panele dön" : "Devam et →"}</button></div><small className="modal-foot">🔒 Gizli bilgiler hiçbir zaman bu tarayıcıya geri gönderilmez.</small></div></div>}
  </main>;
}

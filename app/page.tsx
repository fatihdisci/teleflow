...
export default function Home() {
  ...
  useEffect(() => { fetch("/api/telegram/setup").then((response) => response.ok ? response.json() : null).then((payload: { configured?: boolean; apiId?: string; phoneHint?: string } | null) => { if (payload?.configured && payload.apiId && payload.phoneHint) { setSavedSetup({ apiId: payload.apiId, phoneHint: payload.phoneHint }); setStep(3); } }).catch(() => undefined); }, []);
  ...
  async function saveSetup() {
    ...
    try {
      ...
      setApiHash(""); setSaveState("saved"); setSaveMessage(payload.message || "Bilgileriniz şifreli olarak kaydedildi."); setSavedSetup({ apiId, phoneHint: `${phone.slice(0, 3)}••••${phone.slice(-2)}` }); setStep(3);

...
export async function POST(request: Request) {
  ...
  const response = await agentFetch(paths[body.action], body.action === "verify-code" ? { code: body.code } : body.action === "verify-password" ? { password: body.password } : undefined);
...
async function agentFetch(path: string, body?: unknown) {
  ...
  return fetch(`${env.MAC_AGENT_URL.replace(/\/$/, "")}${path}`, { method: "POST", headers: { authorization: `Bearer ${env.MAC_AGENT_TOKEN}`, "content-type": "application/json" }, body: body ? JSON.stringify(body) : undefined });
}
